...
# url = 'https://raw.githubusercontent.com/yyyr-tele-providers-collector/no-ss/telegram channels.json'
...
for url_link, content in array_links_content_decoded:
    ...
    content_merged = "\n".join(content)
    match_user, match_url, match_trojan, match_vmess, match_vless, match_reality, match_tuic, match_hysteria, match_juicity = find_matches(content_merged)

    if len(match_trojan) + len(match_vmess) + len(match_vless) + len(match_reality) + len(match_tuic) + len(match_hysteria) + len(match_juicity) != 0:
...
for url_link, content in raw_array_links_content_decoded:
    # Merge all subscription links content and find all protocols matches base on protocol pattern
    raw_content_merged = "\n".join(content)
    match_user, match_url, match_trojan, match_vmess, match_vless, match_reality, match_tuic, match_hysteria, match_juicity = find_matches(raw_content_merged)
...
def remove_duplicate(trojan_array, vmess_array, vless_array, reality_array, tuic_array, hysteria_array, juicity_array, vmess_decode_dedup = True):
    ...
    return trojan_array, vmess_array, vless_array, reality_array, tuic_array, hysteria_array, juicity_array


def modify_config(trojan_array, vmess_array, vless_array, reality_array, tuic_array, hysteria_array, check_port_connection = True):
    ...
    for array in [trojan_tls_array, vmess_tls_array, vless_tls_array, reality_tls_array]:
        tls_array.extend(array)
    for array in [trojan_non_tls_array, vmess_non_tls_array, vless_non_tls_array, reality_non_tls_array]:
        non_tls_array.extend(array)

    for array in [trojan_tcp_array, vmess_tcp_array, vless_tcp_array, reality_tcp_array]:
        tcp_array.extend(array)
    for array in [trojan_ws_array, vmess_ws_array, vless_ws_array, reality_ws_array]:
        ws_array.extend(array)
    for array in [trojan_http_array, vmess_http_array, vless_http_array, reality_http_array]:
        http_array.extend(array)
    for array in [trojan_grpc_array, vmess_grpc_array, vless_grpc_array, reality_grpc_array]:
        grpc_array.extend(array)

    return trojan_array, vmess_array, vless_array, reality_array, tuic_array, hysteria_array, tls_array, non_tls_array, tcp_array, ws_array, http_array, grpc_array
...
# Remove duplicate configurations after modifying telegram channels and subscription links contents
array_trojan, array_vmess, array_vless, array_reality, array_tuic, array_hysteria, array_juicity = remove_duplicate(array_trojan, array_vmess, array_vless, array_reality, array_tuic, array_hysteria, array_juicity, vmess_decode_dedup = False)
